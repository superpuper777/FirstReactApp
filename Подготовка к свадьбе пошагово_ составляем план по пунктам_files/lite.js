...
(function staticManagerDefinition() {
  ...
  function addCssFile(_ref) {
    ...
    var path = '/css/' + (vk.css_dir || '') + (stTypes.fromRoot[fileName] || fileName.indexOf('/') !== -1 ? '' : 'al/');
  ...
  var stManager = {
    ...
    add: function add(files, callback) {
      ...
      for (var i in files) {
        ...
        if (isJsFile(file)) {
          jsPath = getJsPath(file);
        }

        if (/^lang\d/i.test(file)) {
          stVersions[file] = stVersions['lang'];
        } else if (!stVersions[jsPath + file]) {
          stVersions[jsPath + file] = 1;
        }

        var fullPath = '/js/' + jsPath + file;
